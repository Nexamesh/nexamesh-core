...
## What NexaMesh Does (Plain English)

NexaMesh is a **defensive system** that detects, classifies, and
responds to hostile small drones on its own when radios and backhaul are jammed.
It makes **policy-bounded, "soft kill first" decisions at the edge** in
{PERFORMANCE.RESPONSE_TIME} (p50) / ≤250 ms (p95) and records tamper-evident
receipts after the fact for accountability.
...
| **Evidence-grade audit** | Fabric: hashes & metadata on chain, full evidence in off-chain WORM stores; ~{BLOCKCHAIN.TPS_SUSTAINED} TPS |
...
- {HARDWARE.POWER_TYPICAL}-{HARDWARE.POWER_PEAK} per node
  ({HARDWARE.COMPUTE_PLATFORM} class)
...
© 2026 NexaMesh. All rights reserved._
