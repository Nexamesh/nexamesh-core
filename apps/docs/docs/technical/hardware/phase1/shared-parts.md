...
- ESP32 LEDC timer resolution is configurable (up to 20-bit duty resolution);
  resolution and frequency trade off. LEDC is software-driven and can be
  affected by WiFi/RTOS activity — unlike the PCA9685's fixed 12-bit hardware
  PWM, which is better for high-precision servo control.
