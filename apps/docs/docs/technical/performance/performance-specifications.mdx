...
- **False Positive Rate**: {PERFORMANCE.FALSE_POSITIVE} target false positive
  rate
- **Detection Range**: {PERFORMANCE.DETECTION_RANGE} depending on sensor
  modality
- **Concurrent Targets**: {PERFORMANCE.CONCURRENT_TARGETS} concurrent drone
  targets
...
- **Autonomous Operation**: {PERFORMANCE.AUTONOMY_LEVEL} edge autonomy without
  network dependency
...
- **Power Consumption**: {HARDWARE.POWER_TYPICAL} typical, {HARDWARE.POWER_PEAK}
  peak
- **Operating Temperature**: {HARDWARE.TEMP_RANGE_INDUSTRIAL} (Industrial
  variants)
...
The NexaMesh Performance Specifications provide comprehensive
...
- **High Performance**: {HARDWARE.COMPUTE_TOPS} AI performance with &lt;200ms
  response times
...
- **Network**: Resilient mesh networking with {TECH.COORDINATION_LATENCY}
  latency
