...
1. **Problem**: Current O(n) Firestore similarity search doesn't scale beyond
   ~500 documents
2. **Decision**: Azure AI Search provides sub-10ms vector search at **$0 cost**
   (covered by existing Azure credits)
3. **Trade-off**: Deeper Azure vendor commitment is acceptable given our
   Azure-native AI strategy
...
**Scale**: ~107 docs, ~500-1K chunks, 100-500 queries/day, growing to 1K docs
over 2 years.

**Key constraint**: We have **free Azure Foundry credits** and already host AI
models there (gpt-4o, gpt-5, text-embedding-3-small).
...
**Final Score**: Azure AI Search scored **93%** in weighted evaluation (10
criteria, 7 options compared).

See
[Appendix: Weighted Decision Matrix](./adr-0011-appendix-vector-db-analysis.md#weighted-decision-matrix)
for full scoring.
...
See
[Appendix: Azure Deployment Alternatives](./adr-0011-appendix-vector-db-analysis.md#azure-deployment-alternatives)
for 7 Azure-specific options evaluated.
...
- **Vendor consolidation**: Deeper Azure commitment (embeddings, indexes,
  metrics, gateway)
- **Cross-cloud latency**: Firebase → Azure adds +50-150ms P99 per query
- **Fallback is partial**: If Azure fails, fallback is keyword-only (no vector
  search)
...
Cognitive Mesh uses vector stores internally within its Reasoning Layer, but
this is an infrastructure-level decision that remains valid regardless of
platform.

**CM Compatibility**: Azure AI Search works with CM via its ReasoningLayer
adapters. No migration needed at this level if platform shift occurs.

**Resource Trade-off Note**: Vector database selection is
infrastructure-agnostic. This ADR's decision carries forward to CM without
rework.
