...
- **Draft** index files, new domains, and referenced docs (runbooks, playbooks,
  EDR, etc.)
- **Promote** when ready: copy or move content from `docs-staging/` into
  `docs/`, then remove or archive this folder
...
- `technical/` — architecture (diagrams, interfaces, subsystems, products), ML,
  hardware, control
...
Each folder has an `index.mdx` as the entry point; add or edit docs under the
...
The Docusaurus app currently reads from `docs/`. To preview this staging content
you can:

1. Temporarily point the preset in `docusaurus.config.ts` at `docs-staging`
   instead of `docs`, or
2. Use a script that copies `docs-staging/*` to a temp dir and runs build from
   there.
